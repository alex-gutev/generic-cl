...
use of the generic function wrappers will be slower than code which
...
compiler-macros which inline replace the calls to the n-argument
functions with multiple calls to the binary functions, e.g. ~(= 1 2
3)~ is replaced with ~(and (equalp 1 2) (equalp 1 3))~.
