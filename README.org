...
Generic function wrappers are provided over all the Common Lisp
sequence operations with methods specialized on ~CL:SEQUENCE~, that
simply call the corresponding function in the ~COMMON-LISP~ package,
and a default ~T~ method that is implemented using the iterator
interface.

Some functions have some differences in the arguments that are
accepted, and a few non-destructive functions are provided for which
there is only a destructive version in the ~COMMON-LISP~ package.

The default value of the ~:TEST~ arguments is ~GENERIC-CL:EQUALP~. The
~:TEST-NOT~ arguments have been removed.

In this section, only the functions functions which differ (in
behavior or arguments) from their ~CL~ counterparts, and functions
which have no analogue in the ~CL~ package, are listed. The
documentation focuses on the differences from the ~CL~ functions. For
a full specification of the functions visit a Common Lisp reference.
...
according the function ~PREDICATE~.
...
***** EVERY, SOME, NOTEVERY AND NOTANY

These are not implemented as generic functions since they take an
arbitrary number of sequences as arguments, however they are
implemented using the iterator interface and will work with any
sequence that implements that interface.

There is no difference in behavior between these functions and the
